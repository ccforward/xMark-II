  ...
  <div class="modal-overlay" @click.self="$emit('close')">
    <div class="modal">
      ...
      <div style="display:flex;gap:8px;margin-bottom:16px">
        <input v-model="newName" class="form-input" placeholder="New category name..." @keyup.enter="create" />
      ...
      <div class="category-list">
        <div v-for="cat in categories" :key="cat.name" class="category-list-item">
          ...
          <div class="actions">
            <button class="icon-btn" title="Rename" @click="rename(cat.name)"><PenLine :size="14" /></button>
            <button class="icon-btn" title="Delete" @click="remove(cat.name)"><Trash2 :size="14" /></button>
          </div>
        </div>
...
function rename(name) {
  const newN = prompt('New category name:', name)
  if (newN && newN !== name) emit('rename', name, newN)
}

function remove(name) {
  if (confirm(`Delete category "${name}"?`)) emit('delete', name)
}
</script>
